...
inline struct timespec ToPosixTimespec(double seconds) {
    struct timespec ts{};
...
void AndroidTaskRunner::scheduleDelayedWakeUp(TimeInterval time) {
    if (time <= 0) {
        return;
    }
    struct itimerspec wake_up = {};
    wake_up.it_value = ToPosixTimespec(time);
    if (timerfd_settime(m_timerFD, TFD_TIMER_ABSTIME, &wake_up, nullptr) == -1) {
...
void AndroidTaskRunner::scheduleImmediateWakeUp() {
    uint64_t value = 1;
    if (write(m_eventFD, &value, sizeof(value)) == -1 && errno != EAGAIN) {
...
void AndroidTaskRunner::postScheduleTask(std::function<void()> task, TimeInterval runtime) {
    bool is_next = false;
    {
        std::lock_guard<std::mutex> lock(m_lock);
